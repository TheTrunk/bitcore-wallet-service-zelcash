...
# Getting Started
```
 git clone https://github.com/TheTrunk/bitcore-wallet-service-zelcash.git
 cd bitcore-wallet-service-zelcash && npm start
```
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/installation.md). 
...
```
  pm2 start app.js --name "bitcoin-wallet-service-zelcash"
```
...
See [Bitcore Wallet Client](https://github.com/TheTrunk/bitcore-wallet-client-zelcash/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v1/addresses/`: Request a new main address from wallet

Returns:
 * Address object: (https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/TheTrunk/bitcore-wallet-service-zelcash/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
